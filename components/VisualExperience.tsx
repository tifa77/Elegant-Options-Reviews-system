...
import {
  ...
  CheckCheck, Zap, ShieldAlert, TrendingUp,
  Signal, Wifi, Battery, Timer, MoveDown,
...
const VisualExperience: React.FC<VisualExperienceProps> = ({ language, data, onBack }) => {
  ...
  // ✅ WhatsApp link: includes project name, project type, and 70% discount request
  const waNumber = '96566305551';

  const projectTypeLabel = (() => {
    const pt = String(data.projectType || '').toLowerCase();
    const ar: any = { restaurant: 'مطعم', cafe: 'كافيه', clinic: 'عيادة', hotel: 'فندق', other: 'أخرى' };
    const en: any = { restaurant: 'Restaurant', cafe: 'Café', clinic: 'Clinic', hotel: 'Hotel', other: 'Hotel', other: 'Other' };
    return isRTL ? (ar[pt] || data.projectType || 'غير محدد') : (en[pt] || data.projectType || 'Unspecified');
  })();

  const waMessage = isRTL
    ? `مرحباً، قمت بتجربة المحاكي وأريد تفعيل نظام Elegant Options.\nاسم المشروع: ${data.projectName}\nنوع المشروع: ${projectTypeLabel}\nوأريد حجز الخصم 70%.`
    : `Hi, I tried the simulator and I want to activate Elegant Options.\nProject name: ${data.projectName}\nProject type: ${projectTypeLabel}\nI want to reserve the 70% discount.`;
  ...
  return (
    <div
      className={`max-w-6xl mx-auto flex flex-col items-center animate-fade-in pb-10 relative font-tajawal ${isRTL ? 'text-right' : 'text-left'}`}
      dir={isRTL ? 'rtl' : 'ltr'}
    >
      ...
      <div className="space-y-4 w-full mb-12 px-4 text-center">
        <h2 className="text-2xl md:text-5xl font-black text-white uppercase tracking-tight leading-tight">
          {isRTL ? 'نظام النمو الذكي وحماية السمعة' : 'Smart Growth System & Reputation Protection'}
        </h2>
        <p className="text-blue-400 text-sm md:text-xl font-bold opacity-80 italic">
          {isRTL
            ? '« حوّل كل عملية بيع إلى جيش من التقييمات الإيجابية وسدّ الباب أمام السلبيات تلقائياً »'
            : '“Turn every sale into a wave of positive reviews and block negatives automatically.”'}
        </p>
      </div>
      ...
      <div className="relative w-full flex flex-col items-center">
        <div className="relative mx-auto w-full max-w-[330px] md:max-w-[360px] h-[680px] md:h-[740px] bg-black rounded-[50px] lg:rounded-[60px] shadow-[0_40px_100px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col z-10 border border-white/10">
          ...
          <div className="flex-1 relative flex flex-col overflow-hidden bg-white rounded-[40px] lg:rounded-[50px] m-1.5 lg:m-2 border border-black/20 mt-10 mb-4">
            ...
            <InternalGuide
              visible={stage === 'chat'}
              title={isRTL ? "الأتمتة اللحظية ⚡" : "Instant Automation ⚡"}
              color="blue"
              icon={Zap}
              text={
                isRTL
                  ? (isRestaurant
                    ? "عند استلام طلب الأكل من شركات التوصيل أو المطعم مباشرة، يرسل النظام هذه الرسالة المخصصة فوراً لضمان الحصول على التقييم في ذروة رضا العميل."
                    : "يقوم النظام بإرسال رسالة مخصصة آلياً فور انتهاء الخدمة لضمان الحصول على التقييم في ذروة رضا العميل.")
                  : "System sends an automated custom message instantly to capture the feedback at peak satisfaction."
              }
            />

            <InternalGuide
              visible={stage === 'survey' && rating === 'sad'}
              title={isRTL ? "درع الحماية النشط 🛡️" : "Active Protection 🛡️"}
              color="red"
              icon={ShieldAlert}
            ...
            <InternalGuide
              visible={stage === 'survey' && rating === 'neutral'}
              title={isRTL ? "إجراء وقائي ذكي ⚠️" : "Precautionary Step ⚠️"}
              color="orange"
              icon={Info}
            ...
            <InternalGuide
              ...
              title={isRTL ? "محرك الهيمنة الرقمية 🚀" : "Digital Dominance 🚀"}
              color="green"
              icon={TrendingUp}
              text={isRTL ? "ستظهر الآن صفحة التقييمات للعميل.. وهذا هو الوقت المناسب وأقوى تحفيز لتقييمك بـ 5 نجوم لتعزيز صدارتك الرقمية." : "The review page appears now—this is the perfect time and strongest motivator for a public 5-star rating."}
            />
            ...
            {stage === 'chat' ? (
                  ...
                  <div className="flex items-center justify-start gap-2 flex-1">
                    ...
                    <div className="text-right overflow-hidden">
                      ...
                      <p className="text-[8px] opacity-80 font-bold">
                        {isRTL ? 'نشط الآن • حساب تجاري' : 'Active now • Business account'}
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex-1 p-4" style={{ backgroundImage: 'url("https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png")', backgroundSize: '400px' }}>
                  {msgVisible && (
                    <div className="bg-white p-3 lg:p-4 rounded-2xl rounded-tl-none shadow-xl max-w-[92%] animate-fade-in-up border border-slate-100 float-right relative mt-52" dir="rtl">
                      ...
                      <p className="text-[12px] lg:text-[13px] text-slate-800 leading-relaxed font-bold text-right relative z-10">
                        {getKuwaitiMessage()}
                      </p>

                      <button
                        onClick={() => setStage('survey')}
                        className="w-full mt-3 bg-blue-600 py-2.5 rounded-xl text-white font-black text-xs lg:text-sm shadow-lg shadow-blue-100 relative z-10"
                      >
                        {isRTL ? 'تقييم التجربة الآن' : 'Rate the experience now'}
                      </button>

                      <div className="flex justify-end mt-1 items-center gap-0.5 text-[#53bdeb] relative z-10">
                        <span className="text-[9px] text-slate-400 font-bold">1:21 PM</span> <CheckCheck size={12} />
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="flex-1 flex flex-col bg-white h-full overflow-hidden p-4 lg:p-6 items-center text-center mt-36" dir="rtl">
                ...
                <div className="mb-6 px-2">
                  <h3 className="font-black text-slate-800 text-base lg:text-lg mb-1">
                    {isRTL ? 'كيف كانت تجربتك معانا؟' : 'How was your experience?'}
                  </h3>
                  <p className="text-[9px] text-slate-500 font-bold leading-tight">
                    {isRTL
                      ? `مشروع **${data.projectName}** يطلب مشاركة تجربتك بكل شفافية`
                      : `**${data.projectName}** would like you to share your experience honestly.`}
                  </p>
                </div>
                ...
                <div className="flex justify-between w-full mb-8 px-2">
                  <button onClick={() => setRating('happy')} className={`group flex flex-col items-center gap-2 transition-all ${rating === 'happy' ? 'scale-110' : 'opacity-30 grayscale'}`}>
                    ...
                    <span className="text-[10px] font-black text-green-600">{isRTL ? 'ممتازة' : 'Excellent'}</span>
                  </button>
                  ...
                  <button onClick={() => setRating('neutral')} className={`group flex flex-col items-center gap-2 transition-all ${rating === 'neutral' ? 'scale-110' : 'opacity-30 grayscale'}`}>
                    ...
                    <span className="text-[10px] font-black text-yellow-600">{isRTL ? 'متوسطة' : 'Average'}</span>
                  </button>
                  ...
                  <button onClick={() => setRating('sad')} className={`group flex flex-col items-center gap-2 transition-all ${rating === 'sad' ? 'scale-110' : 'opacity-30 grayscale'}`}>
                    ...
                    <span className="text-[10px] font-black text-red-600">{isRTL ? 'سيئة' : 'Bad'}</span>
                  </button>
                </div>
                ...
                {rating && !isSent && (
                  <div className="w-full animate-fade-in-up space-y-3 px-2">
                    ...
                    <button
                      ...
                      className={`w-full py-3 rounded-xl font-black text-xs flex items-center justify-center gap-2 shadow-xl transform active:scale-95 transition-transform ${rating === 'happy' ? 'bg-[#4285F4] text-white' : 'bg-slate-900 text-white'}`}
                    >
                      {rating === 'happy' ? (
                        <>
                          <img
                    ...
                    {rating === 'happy' ? (
                      <>
                        <h4 className="text-xs font-black text-slate-800 mb-1">
                          {isRTL ? 'ستظهر صفحة التقييمات الآن' : 'The review page will open now'}
                        </h4>
                        <p className="text-[8px] text-green-600 font-bold leading-tight">
                          {isRTL
                            ? 'هذا هو أقوى تحفيز والوقت المناسب للعميل لتقييمك بـ 5 نجوم لتعزيز صدارتك الرقمية.'
                            : 'This is the best timing and strongest motivator to get a 5-star public rating.'}
                        </p>
                      </>
                    ) : (
                      <>
                        <h4 className="text-xs font-black text-slate-800 mb-1">
                          {isRTL ? 'تم توجيه ملاحظتك للادارة' : 'Your feedback was sent to management'}
                        </h4>
                        <p className="text-[8px] text-red-500 font-bold leading-tight">
                          {isRTL
                            ? 'سيتم التواصل معك والحرص على رضاء جميع العملاء.'
                            : 'We will contact you and ensure your satisfaction privately.'}
                        </p>
                      </>
                    )}

                    <button
                      onClick={() => setStage('post-demo')}
                      className="mt-4 text-[10px] font-black text-blue-500 uppercase tracking-widest underline decoration-2 underline-offset-4"
                    >
                      {isRTL ? "إنهاء العرض" : "Finish Demo"}
                    </button>
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 bg-white h-full text-center mt-32" dir="rtl">
                <div className="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center mb-4 border border-green-100 shadow-inner">
                  <CheckCheck size={32} className="text-green-600 animate-bounce" />
                </div>

                <h2 className="text-lg font-black text-slate-900 mb-2 leading-tight uppercase tracking-tighter">
                  {isRTL
                    ? 'نظام ELEGANT OPTIONS: حماية مستمرة لا تتوقف'
                    : 'ELEGANT OPTIONS: Continuous Protection That Never Stops'}
                </h2>

                <p className="text-slate-600 text-[10px] font-bold leading-relaxed px-2 mb-6">
                  {isRTL
                    ? 'يضمن لك صدارة البحث وحماية سمعتك الرقمية تلقائياً وبشكل مستمر على مدار الساعة.'
                    : 'It secures your search dominance and protects your reputation automatically—24/7.'}
                </p>
      ...
      <div className="w-full max-w-[420px] space-y-4 mt-8 lg:mt-12 px-4 flex flex-col items-center">
        <div className="bg-red-600/10 border border-red-500/30 py-2 px-6 rounded-full inline-flex items-center gap-3 animate-pulse">
          ...
          <span className="text-[10px] lg:text-[11px] font-black text-white tracking-widest uppercase">
            {isRTL ? "احجز الخصم الخاص بك:" : "Claim Your Discount:"}{' '}
            <span className="font-mono text-base ml-1">{formatTime(timeLeft)}</span>
          </span>
        </div>

        <div className="relative group w-full">
          <div className="absolute -top-4 z-20 bg-yellow-400 text-black font-black text-[10px] px-3 py-1 rounded-lg shadow-[0_0_20px_rgba(250,204,21,0.8)] animate-pulse border-2 border-black rotate-2 right-0">
          ...
          <a
            ...
            className="relative block w-full py-4 lg:py-5 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-black text-xl lg:text-2xl rounded-[1.5rem] lg:rounded-[2rem] shadow-2xl text-center transform hover:-translate-y-1 transition-all border-b-4 border-green-800"
          >
            <span className="flex items-center justify-center gap-2 lg:gap-3 uppercase tracking-tighter">
              <Zap className="fill-yellow-300 text-yellow-300 w-5 h-5 lg:w-7 lg:h-7" />
              {isRTL ? 'احصل على النظام الآن' : 'Get the system now'}
            </span>
          </a>
        </div>
        ...
        <button
          onClick={onBack}
          className="w-full py-2 text-slate-500 font-bold hover:text-white transition-all flex items-center justify-center gap-2 text-[10px] uppercase tracking-widest opacity-60"
        >
