...
import { 
  ...
  MapPin, Hotel, CheckCircle2, Activity
} from 'lucide-react';
...
const DataIntake: React.FC<DataIntakeProps> = ({ onStart, language }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // محاكاة تقنية سريعة (600ms) لضمان الربط السلس مع التقرير
    setTimeout(() => {
  ...
  return (
    <div className={`max-w-3xl mx-auto animate-fade-in pb-12 ${isRTL ? 'font-tajawal text-right' : 'font-sans text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="text-center mb-10">
         <img src="https://storage.googleapis.com/msgsndr/vX7gQQOe9PXtkGes2GOJ/media/6944362aa49c0a6975236470.png" alt="Elegant Options" className="w-20 h-20 object-contain mx-auto mb-6 transition-transform hover:scale-110" />
      ...
      <div className="bg-[#050a12] border-2 border-solid border-white/10 rounded-[3rem] p-8 md:p-12 shadow-[0_40px_100px_rgba(0,0,0,0.8)] relative">
        <form onSubmit={handleSubmit} className="space-y-10 relative z-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {types.map((type) => (
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-5 rounded-3xl border-2 border-solid transition-all duration-300 ${
                    projectType === type.id 
                ...
                >
                  <type.icon size={26} className={`mb-2 ${projectType === type.id ? 'text-blue-400' : 'text-slate-600'}`} />
          ...
          {/* الخريطة بالأبيض والأسود - استجابة فورية */}
          <div className="space-y-4">
             <div className="flex items-center gap-2 text-[13px] font-black text-blue-400 uppercase tracking-widest">
               <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></div>
               {isRTL ? 'تحديد الموقع الرقمي (بانتظار الاسم)' : 'DIGITAL LOCATION (AWAITING NAME)'}
            </div>
            <div className="h-48 bg-slate-900 rounded-[2.5rem] border-2 border-solid border-white/5 relative flex items-center justify-center overflow-hidden grayscale">
               ...
               <div className="relative z-10 flex flex-col items-center gap-3">
                  ...
                  <span className="text-[11px] font-black text-white uppercase tracking-tighter bg-black/60 px-5 py-2 rounded-full backdrop-blur-md border border-white/10 shadow-xl">
                    {isRTL ? 'سيتم تحديد مشروعك بمجرد كتابة الاسم' : 'PROJECT WILL BE LOCATED UPON NAMING'}
                  </span>
               </div>
               <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="w-64 h-64 border-2 border-blue-500/10 rounded-full animate-ping"></div>
               </div>
            </div>
          </div>
          ...
          {/* شبكة البيانات مع الشرح التفصيلي */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             <div className="space-y-3">
                <div className="bg-[#0a121e] border-2 border-solid border-white/5 p-6 rounded-[2rem] transition-colors hover:border-blue-500/30">
                   <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 mb-2"><Calendar size={16} className="text-blue-500"/> {isRTL ? 'سنة التأسيس' : 'EST. YEAR'}</span>
                ...
                <p className="text-[11px] text-slate-500 font-black px-2 leading-tight">{isRTL ? 'يجب وضع هذه البيانات لتحليل عمر مشروعك في السوق بشكل صحيح.' : 'Provide this to analyze your market lifespan correctly.'}</p>
             </div>
             
             <div className="space-y-3">
                <div className="bg-[#0a121e] border-2 border-solid border-white/5 p-6 rounded-[2rem] transition-colors hover:border-blue-500/30">
                   <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 mb-2"><Star size={16} className="text-blue-500"/> {isRTL ? 'إجمالي التقييمات' : 'TOTAL REVIEWS'}</span>
                ...
                <p className="text-[11px] text-slate-500 font-black px-2 leading-tight">{isRTL ? 'يجب وضع هذه البيانات لتقييم مصداقيتك الحالية في جوجل بشكل صحيح.' : 'Provide this to evaluate your Google credibility correctly.'}</p>
             </div>

             <div className="space-y-3">
                <div className="bg-[#0a121e] border-2 border-solid border-white/5 p-6 rounded-[2rem] transition-colors hover:border-blue-500/30">
                   <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 mb-2"><Users size={16} className="text-blue-500"/> {isRTL ? 'العملاء يومياً' : 'DAILY CUSTOMERS'}</span>
                ...
                <p className="text-[11px] text-slate-500 font-black px-2 leading-tight">{isRTL ? 'يجب وضع هذه البيانات لتحليل فرص النمو الضائعة يومياً بشكل صحيح.' : 'Provide this to analyze daily missed growth correctly.'}</p>
             </div>
          </div>
