...
const DataIntake: React.FC<DataIntakeProps> = ({ language, onSubmit, onBack }) => {
  ...
  const [formData, setFormData] = useState<AuditData>({
    ...
    customProjectType: '', // حقل إضافي لنوع النشاط المخصص
  ...
  // تحديث الخريطة تلقائياً عند تغيير اسم المشروع أو النوع
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // رابط مصحح لخرائط جوجل يدعم اللغة
      setMapUrl(`http://googleusercontent.com/maps.google.com/7{encodedQuery}&hl=${isRTL ? 'ar' : 'en'}&t=&z=13&ie=UTF8&iwloc=&output=embed`);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // إذا اختار "أخرى"، ندمج النوع المخصص في الإرسال
    const finalData = {
      ...formData,
  ...
  return (
    <div className={`max-w-4xl mx-auto relative ${isRTL ? 'text-right font-tajawal' : 'text-left font-sans'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {/* Logo Header */}
      <div className="flex flex-col items-center justify-center mb-8 animate-fade-in-up pt-8 text-center">
      ...
      <div className="bg-slate-800/80 backdrop-blur-md rounded-3xl shadow-2xl border border-slate-700/50 p-6 md:p-8">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-3">
            ...
            {/* حقل النشاط المخصص (يظهر عند اختيار "أخرى") */}
            ...
            <div className="space-y-2">
               ...
               <div className="relative">
                 <input 
                    ...
                    placeholder={isRTL ? "مثال: مطعم فتوش السالمية" : "Ex: Fatoush Restaurant Salmiya"} 
                 />
          ...
          {/* 3. Bottom Grid (Fixed and Aligned) */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            
            {/* سنة التأسيس */}
            <div className="bg-slate-900/40 p-4 rounded-2xl border border-slate-700/50 flex flex-col group">
              <label className="text-[10px] uppercase font-bold text-slate-500 mb-2 min-h-[35px] flex items-end">
