...
const ResultsDashboard: React.FC<ResultsDashboardProps> = ({ language, data, onReset, onBack, onVisualExp }) => {
  ...
  // معادلة التأثير والقيمة المالية
  ...
  // 4. نظام التصنيف الصارم الجديد (Strict SEO Logic)
  ...
  return (
    <div className={`max-w-4xl mx-auto space-y-8 animate-fade-in pb-20 relative ${isRTL ? 'font-tajawal text-right' : 'font-sans text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-6 rounded-[2rem] border border-orange-500/30 relative overflow-hidden group hover:border-orange-500/50 transition-all">
            <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
                <div className="flex items-center -space-x-4 rtl:space-x-reverse">
                    <div className="w-16 h-16 rounded-2xl bg-[#ff5a00] flex items-center justify-center border-4 border-slate-900 z-20"><Bike className="text-white w-8 h-8" /></div>
                    <div className="w-16 h-16 rounded-2xl bg-[#fec400] flex items-center justify-center border-4 border-slate-900 z-10"><Zap className="text-black w-8 h-8 fill-black" /></div>
                </div>
                <div className="flex-1 text-center md:text-right">
      ...
      <div className="pt-8 space-y-4">
        ...
        <button onClick={onReset} className="w-full py-4 text-slate-500 hover:text-slate-300 text-sm font-medium flex items-center justify-center gap-2"><RotateCw size={14} /> {t.closing.btn2}</button>
      </div>
