...
// استيراد شامل ومنظم لكافة الأيقونات المستخدمة لضمان عدم حدوث ReferenceError
import { 
  TrendingUp, 
  TrendingDown, 
  AlertTriangle, 
...
const ResultsDashboard: React.FC<ResultsDashboardProps> = ({ language, data, onReset, onBack, onVisualExp }) => {
  ...
  const lostCustomersCount = Math.max(0, (projectedYearlyReviews - (currentMonthly * 12)) * customerLossMultiplier);
  ...
  const getMarketStatus = () => {
    ...
    if (avgReviewsPerYear < 15) {
      ...
      return { 
        ...
        desc: isRTL ? "أنت موجود ولكنك مهدد. المنافسون يبتلعون حصتك السوقية تدريجياً لأنهم أكثر نشاطاً منك." : "You are present but at risk. Competitors are eating your market share.",
    ...
    return { 
      ...
      desc: isRTL ? "أداء ممتاز، ولكن الحفاظ على القمة أصعب من الوصول إليها. المنافسون يتربصون بك." : "Good performance, but staying on top is harder. Competitors are watching.",
  ...
  return (
    <div className={`max-w-5xl mx-auto space-y-16 animate-fade-in pb-32 ${isRTL ? 'font-tajawal text-right' : 'font-sans text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex items-center justify-between px-2 pt-4 text-white">
        ...
        <span className="bg-indigo-500/10 border border-indigo-500/20 px-4 py-1 rounded-full text-[10px] font-black text-indigo-400 uppercase tracking-widest">Growth Intelligence</span>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: isRTL ? 'عمر النشاط' : 'Age', value: ageYears, sub: isRTL ? 'سنوات' : 'Years', icon: Globe },
          ...
          <div key={i} className="bg-slate-900/50 p-6 rounded-[2.5rem] border border-slate-800 text-center group hover:border-indigo-500/30 transition-all">
      ...
      {/* Comparison & Leak Section */}
      <div className="grid md:grid-cols-2 gap-8 text-white">
        <div className="bg-slate-900/80 p-10 rounded-[3rem] border border-slate-800 relative">
           <div className="flex items-center gap-4 mb-8 border-b border-slate-800 pb-6">
             <TrendingDown className="text-red-500" size={28} />
             <h3 className="text-slate-300 font-black text-xl uppercase tracking-tighter">{isRTL ? "الوضع اليدوي الحالي" : "Manual Status"}</h3>
           </div>
           <div className="space-y-6">
             <div className="flex justify-between items-end">
               <span className="text-slate-500 text-sm font-bold uppercase">{isRTL ? "النمو الأسبوعي" : "Weekly Growth"}</span>
             ...
             <p className="text-red-400 text-sm font-bold italic leading-relaxed border-t border-slate-800/50 pt-4">
               {isRTL ? "⚠️ المنافسون يستغلون ضعف الوجود الرقمي ويزيدون أرباحهم." : "⚠️ Competitors are exploiting this weak presence."}
             </p>
           </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-indigo-950/40 to-slate-900 p-10 rounded-[3rem] border-2 border-indigo-500/30 relative overflow-hidden shadow-2xl">
           ...
           <div className="flex items-center gap-4 mb-8 border-b border-indigo-500/20 pb-6 text-white">
             ...
             <h3 className="text-white font-black text-xl uppercase tracking-tighter">With Elegant Options PRO</h3>
           </div>
           <div className="space-y-6">
             <div className="flex justify-between items-end">
               <span className="text-indigo-200 text-sm font-bold uppercase">{isRTL ? "المعدل الإضافي المتوقع" : "Projected Growth"}</span>
             ...
             <div className="pt-6 border-t border-indigo-500/20 flex justify-between items-center">
               <span className="text-indigo-300 text-xs font-black uppercase">{isRTL ? "زيادة سنوية تقديرية" : "Annual Increase"}</span>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3.5rem] space-y-8 hover:border-indigo-500/30 transition-all flex flex-col shadow-2xl">
          <div className="w-16 h-16 bg-indigo-500/10 rounded-2xl flex items-center justify-center text-indigo-400 shrink-0"><Bot size={40} /></div>
          <h4 className="text-2xl font-black text-white italic tracking-tight">AI Smart Replies</h4>
          <p className="text-slate-400 text-lg leading-relaxed flex-1">رد آلي وذكي على كافة التقييمات في Google Maps على مدار 24 ساعة، مما يحسن السيو ويُشعر العميل بالاهتمام الفوري.</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3.5rem] space-y-8 hover:border-orange-500/30 transition-all flex flex-col shadow-2xl">
          <div className="w-16 h-16 bg-orange-500/10 rounded-2xl flex items-center justify-center text-orange-400 shrink-0"><ShieldCheck size={40} /></div>
          <h4 className="text-2xl font-black text-white italic tracking-tight">Reputation Shield</h4>
          <p className="text-slate-400 text-lg leading-relaxed flex-1">فلترة ذكية تمنع ظهور أي تقييم (3 نجوم أو أقل) علناً، وتحويله فوراً لرسالة خاصة للإدارة لمعالجة المشكلة بخصوصية تامة.</p>
        </div>
        <div className={`bg-slate-900 border border-slate-800 p-12 rounded-[3.5rem] space-y-8 hover:border-green-500/30 transition-all flex flex-col shadow-2xl ${!isRestaurant && 'opacity-50 grayscale'}`}>
          <div className="w-16 h-16 bg-green-500/10 rounded-2xl flex items-center justify-center text-green-400 shrink-0"><Bike size={40} /></div>
          <h4 className="text-2xl font-black text-white italic tracking-tight">Delivery Integration</h4>
          <p className="text-slate-400 text-lg leading-relaxed flex-1">ربط مباشر مع طلبات وكيتا؛ إرسال رسائل طلب تقييم تلقائية عبر واتساب فور استلام الطلب، مما يضاعف تقييماتك آلياً.</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border-2 border-indigo-500/20 rounded-[4rem] p-12 md:p-16 relative overflow-hidden shadow-3xl">
          ...
          <div className="flex flex-col md:flex-row items-center gap-12 relative z-10">
              <div className="flex-1 space-y-8 text-white">
                  ...
                  <h4 className="text-4xl md:text-5xl font-black italic">{isRTL ? "أرباح إضافية بانتظار تفعيلها" : "Hidden Profits Waiting"}</h4>
              ...
              <div className="bg-slate-800/50 p-14 rounded-[3.5rem] border border-indigo-500/30 text-center shadow-3xl min-w-[340px] transform hover:scale-105 transition-transform backdrop-blur-xl">
                  ...
                  <span className="text-slate-500 text-xs font-black uppercase tracking-widest block mb-6">{isRTL ? "أرباح سنوية إضافية محتملة" : "Potential Additional Annual Profit"}</span>
      ...
      <div className="flex flex-col md:flex-row gap-8 justify-center pt-16">
          <button onClick={onVisualExp} className="px-16 py-8 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-3xl rounded-[3rem] shadow-2xl flex items-center justify-center gap-5 transition-all transform hover:-translate-y-1">
              <Play size={32} fill="currentColor" /> {isRTL ? "تجربة بصرية" : "Visual Experience"}
          </button>
          <a href={waLink} target="_blank" rel="noopener noreferrer" className="px-16 py-8 bg-green-500 hover:bg-green-600 text-white font-black text-4xl rounded-[3rem] shadow-2xl flex items-center justify-center gap-6 transition-all animate-pulse transform hover:-translate-y-1">
              <MessageCircle size={36} /> {isRTL ? "اطلب النظام الآن" : "Order System"}
          </a>
      </div>

      <div className="text-center pt-10">
        <button onClick={onReset} className="inline-flex items-center gap-4 text-slate-500 hover:text-white font-black text-2xl transition-all opacity-50 hover:opacity-100">
