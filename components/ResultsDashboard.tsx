...
const ResultsDashboard: React.FC<ResultsDashboardProps> = ({ language, data, onReset, onBack, onVisualExp }) => {
  ...
  // --- 1. محرك الأمان الرقمي والحسابات (Baseline Logic) ---
  ...
  // Baseline Review Rates (Current Reality)
  ...
  const systemDailyPotential = Math.round(dailyCustomers * 0.10); // 10% Rule
  ...
  // Lost Metrics (The Gap)
  const lostDailyReviews = Math.max(0, systemDailyPotential - baselineDaily);
  const lostWeeklyReviews = Math.max(0, systemWeekly - baselineWeekly);
  ...
  // Growth & Profit Calculations
  const percentageIncrease = avgReviewsPerYear > 0 
    ? Math.round(((systemYearly - avgReviewsPerYear) / avgReviewsPerYear) * 100) 
    : 100;
  
  const customerLossMultiplier = 4;
  const lostCustomersCount = Math.max(50, (systemYearly - avgReviewsPerYear) * customerLossMultiplier);
  ...
  return (
    <div className={`max-w-5xl mx-auto space-y-16 animate-fade-in pb-32 ${isRTL ? 'font-tajawal text-right' : 'font-sans text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className={`p-10 md:p-14 rounded-[3rem] border ${status.border} ${status.bg} backdrop-blur-md relative overflow-hidden group shadow-2xl`}>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
          ...
          <div className="flex-1 space-y-4">
            ...
            <p className="text-slate-200 text-lg md:text-2xl font-bold leading-relaxed opacity-90">{status.desc}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: isRTL ? 'عمر النشاط' : 'Business Age', value: ageYears, sub: isRTL ? 'سنة' : 'Years', icon: Globe },
          ...
          <div key={i} className="bg-slate-900/50 p-8 rounded-[2.5rem] border border-slate-800 text-center hover:border-slate-700 transition-all group shadow-lg">
      ...
      {/* 3. PERFORMANCE COMPARISON (LONG LAYOUT) */}
      <div className="space-y-10">
        {/* MANUAL STATUS CARD */}
        <div className={`bg-slate-900/80 p-10 md:p-14 rounded-[3.5rem] border-2 ${status.border} relative group overflow-hidden shadow-2xl`}>
           <div className="flex flex-col md:flex-row justify-between gap-10 relative z-10">
              <div className="flex-1 space-y-6">
                <div className="flex items-center gap-4 border-b border-white/5 pb-6">
                  {status.id === 'strong' ? <TrendingUp className="text-green-500" size={32} /> : status.id === 'average' ? <Target className="text-yellow-500" size={32} /> : <TrendingDown className="text-red-500" size={32} />}
                ...
                <p className="text-slate-400 text-xl font-medium leading-relaxed italic max-w-3xl">
                  {status.id === 'strong' 
                    ? (isRTL ? "أداؤك الحالي جيد جداً، لكن الاعتماد على الجهد اليدوي يجعل الحفاظ على هذه القمة أمراً مرهقاً ومعرضاً للتراجع أمام أتمتة المنافسين." : "Your current performance is great, but relying on manual effort makes staying at the top exhausting and vulnerable to automated competitors.")
                    : (isRTL ? status.desc : status.desc)}
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-4">
                  <div className="bg-black/20 p-4 rounded-2xl border border-white/5">
                    <span className="text-slate-500 text-xs block mb-1 uppercase font-black tracking-widest">{isRTL ? "المعدل اليومي" : "Daily Rate"}</span>
                    <span className="text-2xl font-black text-white">{baselineDaily}</span>
                  </div>
                  <div className="bg-black/20 p-4 rounded-2xl border border-white/5">
                    <span className="text-slate-500 text-xs block mb-1 uppercase font-black tracking-widest">{isRTL ? "المعدل الأسبوعي" : "Weekly Rate"}</span>
                    <span className="text-2xl font-black text-white">{baselineWeekly}</span>
                  </div>
                  <div className="bg-black/20 p-4 rounded-2xl border border-white/5">
                    <span className="text-slate-500 text-xs block mb-1 uppercase font-black tracking-widest">{isRTL ? "المعدل الشهري" : "Monthly Rate"}</span>
              ...
              <div className="w-full md:w-80 bg-red-500/5 rounded-3xl p-8 border border-red-500/20 flex flex-col justify-center text-center space-y-4">
                ...
                <h4 className="text-red-500 font-black text-sm uppercase tracking-widest">{isRTL ? "فرص مفقودة" : "Lost Opportunities"}</h4>
                <div className="space-y-1">
                  <div className="text-5xl font-black text-white">-{lostDailyReviews}</div>
                  <p className="text-red-400/80 text-xs font-bold uppercase tracking-widest">{isRTL ? "تقييم تفقده يومياً" : "Reviews lost per day"}</p>
                </div>
        ...
        {/* ELEGANT OPTIONS PRO - LANDING STYLE */}
        <div className="bg-gradient-to-br from-indigo-950/60 to-slate-900 p-10 md:p-16 rounded-[4rem] border-4 border-indigo-500/30 relative overflow-hidden shadow-indigo-500/20 shadow-2xl group">
           ...
           <div className="relative z-10 space-y-12">
              <div className="flex flex-col md:flex-row items-center justify-between gap-8 border-b border-indigo-500/20 pb-10">
                <div className="space-y-4 text-center md:text-left">
                  <div className="flex items-center gap-4 justify-center md:justify-start">
                    ...
                    <h3 className="text-white font-black text-4xl md:text-5xl uppercase tracking-tighter italic">With Elegant Options <span className="text-indigo-400">PRO</span></h3>
                  </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                  { label: isRTL ? "المعدل اليومي" : "PRO Daily Potential", value: `+${systemDailyPotential}`, sub: isRTL ? "تقييم / يوم" : "reviews / day" },
                  { label: isRTL ? "المعدل الأسبوعي" : "PRO Weekly Potential", value: `+${systemWeekly}`, sub: isRTL ? "تقييم / أسبوع" : "reviews / week" },
                  { label: isRTL ? "المعدل الشهري" : "PRO Monthly Potential", value: `+${systemMonthly}`, sub: isRTL ? "تقييم / شهر" : "reviews / month" },
                  { label: isRTL ? "الرصيد السنوي الإضافي" : "PRO Annual Asset", value: `+${systemYearly.toLocaleString()}`, sub: isRTL ? "تقييم جديد سنوياً" : "new reviews yearly", color: "text-green-400" }
                ].map((p, i) => (
                  <div key={i} className="bg-white/5 backdrop-blur-md p-6 rounded-3xl border border-white/10 text-center">
                    <span className="text-indigo-300 text-[10px] uppercase font-black tracking-widest block mb-3">{p.label}</span>
                    <span className={`text-4xl font-black ${p.color || 'text-white'}`}>{p.value}</span>
                    <span className="text-slate-500 text-[10px] block mt-2 font-bold uppercase">{p.sub}</span>
                  </div>
                ))}
              </div>
           </div>
      ...
      <div className="py-20 border-y border-slate-800/50 relative">
        <div className="max-w-4xl mx-auto text-center space-y-12">
           ...
           <div className="grid md:grid-cols-2 gap-12 text-right rtl:text-right ltr:text-left">
              <div className="space-y-6">
                <p className="text-slate-300 text-xl leading-relaxed">
                  {isRTL 
                    ...
                    : "When automation is active, your business transforms into a 'Digital Authority'. You no longer wait for chance reviews; instead, positive feedback flows continuously, signaling strong trust to Google."}
                </p>
                <p className="text-slate-400 text-lg leading-relaxed">
                  {isRTL 
                    ...
                    : "This dense presence boosts your Ranking automatically, ensuring your name appears to new customers the moment they search for your service."}
                </p>
              </div>
              <div className="bg-indigo-500/5 p-10 rounded-[3rem] border border-indigo-500/10 space-y-8">
                 {[
                   { ar: "ظهور مضاعف في الصفحة الأولى لجوجل", en: "Double visibility on Google's first page" },
                   { ar: "حماية مطلقة من التقييمات السلبية الصامتة", en: "Absolute protection from silent negative feedback" },
                   { ar: "تحويل العملاء العابرين إلى ولاء مستدام", en: "Turning casual visitors into sustainable loyalty" }
                 ].map((item, idx) => (
                   <div key={idx} className="flex items-center gap-4">
                     <div className="bg-indigo-500 rounded-full p-1 shrink-0"><CheckCircle className="text-white" size={20} /></div>
      ...
      <div className="bg-slate-900 border-2 border-indigo-500/20 rounded-[4rem] p-12 md:p-16 relative overflow-hidden shadow-3xl group">
          ...
          <div className="flex flex-col md:flex-row items-center gap-12 relative z-10">
              <div className="flex-1 space-y-8 text-white">
                  ...
                  <h4 className="text-4xl md:text-5xl font-black italic">{isRTL ? "أرباح إضافية بانتظار تفعيلها" : "Hidden Profits Unlocked"}</h4>
                  <p className="text-slate-400 text-xl md:text-2xl font-medium leading-relaxed italic border-l-4 border-indigo-500/30 pl-8">
                    {t.dashboard?.marketing?.persuasive || "Elegant Options protects your loyalty, prevents silent churn, and turns every customer into an advocate."}
                  </p>
              </div>
              ...
              <div className="bg-slate-800/50 p-14 rounded-[3.5rem] border border-indigo-500/30 text-center shadow-3xl min-w-[340px] transform hover:scale-105 transition-transform backdrop-blur-xl">
                  ...
                  <span className="text-slate-500 text-xs font-black uppercase tracking-[0.2em] block mb-6">{isRTL ? "الأرباح السنوية الإضافية المحتملة" : "Potential Additional Annual Profit"}</span>
                  <div className="flex flex-col items-center">
      ...
      <div className="space-y-16 py-10">
          <div className="flex flex-col items-center text-center space-y-8 max-w-4xl mx-auto px-4">
             ...
             <p className="text-slate-200 text-3xl md:text-4xl font-black italic leading-tight tracking-tight">
               "{t.dashboard?.quote?.text || "A one-star increase in rating leads to a 5% to 9% increase in revenue."}"
             </p>
          ...
          <div className="bg-indigo-600/5 border border-indigo-500/20 rounded-[4rem] p-12 md:p-20 relative overflow-hidden group shadow-2xl">
             ...
             <div className="relative z-10 space-y-8">
                <div className="flex items-center gap-5 text-indigo-400">
                   <ShieldCheck size={48} />
                   <h3 className="text-3xl md:text-4xl font-black uppercase tracking-tight italic">{t.dashboard?.strategicRecommendation?.title || "Strategic Recommendation"}</h3>
                </div>
      ...
      <div className="text-center space-y-16 pt-16 border-t border-slate-800">
         <div className="space-y-6">
            ...
            <p className="text-indigo-500 text-3xl md:text-4xl font-black animate-pulse uppercase tracking-widest leading-none">
              {t.dashboard?.marketing?.motivational || "Every unmanaged review is a customer lost forever."}
            </p>
         </div>
