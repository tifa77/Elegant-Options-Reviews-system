...
const ResultsDashboard: React.FC<ResultsDashboardProps> = ({ language, data, onReset, onBack, onVisualExp }) => {
  ...
  const ageYears =
    (Number.isFinite(rawEstablishedYear) && rawEstablishedYear > 1900 && rawEstablishedYear <= currentYear)
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-end">
              <span className="text-indigo-200 text-sm font-bold uppercase">
                {isRTL ? 'المعدل الإضافي المتوقع' : 'Projected Growth'}
              </span>
            ...
            <div className="pt-6 border-t border-indigo-500/20 flex justify-between items-center">
              ...
              <div className="bg-green-500 text-black px-4 py-1 rounded-full font-black text-sm">
                +{percentageIncrease}%{/* ✅ المتغير الآن معرّف */}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3.5rem] space-y-8 hover:border-indigo-500/30 transition-all flex flex-col shadow-2xl">
          ...
          <p className="text-slate-400 text-lg leading-relaxed flex-1">
            رد آلي وذكي على كافة التقييمات في Google Maps على مدار 24 ساعة، مما يحسن
            السيو ويُشعر العميل بالاهتمام الفوري.
          </p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-12 rounded-[3.5rem] space-y-8 hover:border-orange-500/30 transition-all flex flex-col shadow-2xl">
          ...
          <p className="text-slate-400 text-lg leading-relaxed flex-1">
            فلترة ذكية تمنع ظهور أي تقييم (3 نجوم أو أقل) علناً، وتحويله فوراً لرسالة
            خاصة للإدارة لمعالجة المشكلة بخصوصية تامة.
          </p>
        </div>
        <div
        ...
        >
          ...
          <p className="text-slate-400 text-lg leading-relaxed flex-1">
            ربط مباشر مع طلبات وكيتا؛ إرسال رسائل طلب تقييم تلقائية عبر واتساب فور
            استلام الطلب، مما يضاعف تقييماتك آلياً.
          </p>
        </div>
      </div>
