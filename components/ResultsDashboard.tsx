...
const ResultsDashboard: React.FC<AuditDashboardProps> = ({ data, language, onReset, onShowVisualExperience }) => {
  ...
  // إعدادات الواتساب الرسمية للشركة
  const whatsappNumber = "96566305551";
  const whatsappMessage = encodeURIComponent(isRTL ? "مهتم لطلب نظام لمشروعي" : "Interested in ordering a system for my project");
  ...
  return (
    <div className={`space-y-12 pb-24 animate-fade-in ${isRTL ? 'font-tajawal text-right' : 'font-sans text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {/* 4. شرح المميزات التنافسية (تنسيق أنيق ومرتب) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {/* ميزة AI */}
        <div className="bg-[#0a121e] border-2 border-blue-500/20 rounded-[3rem] p-10 space-y-6 hover:border-blue-500/50 transition-all group shadow-2xl">
          <div className="flex justify-between items-center">
            <h4 className="text-white font-black text-2xl leading-tight">{isRTL ? 'ردود آلية متطورة بواسطة AI' : 'Advanced AI Replies'}</h4>
        ...
        {/* درع الحماية */}
        <div className="bg-[#0a121e] border-2 border-orange-500/20 rounded-[3rem] p-10 space-y-6 hover:border-orange-500/50 transition-all shadow-2xl">
           <div className="flex justify-between items-center">
        ...
        {/* دمج التوصيل */}
        <div className="bg-[#0a121e] border-2 border-red-500/20 rounded-[3rem] p-10 space-y-6 hover:border-red-500/50 transition-all shadow-2xl">
           <div className="flex justify-between items-center">
            <h4 className="text-white font-black text-2xl leading-tight">{isRTL ? 'دمج تطبيقات التوصيل (طلبات/كيتا)' : 'Delivery App Integration'}</h4>
      ...
      {/* 6. الأرباح السنوية المحققة عبر ولاء العملاء */}
      <div className="bg-[#0a121e] border-2 border-blue-500/30 rounded-[3.5rem] p-12 relative overflow-hidden shadow-2xl">
        <div className="flex flex-col md:flex-row justify-between items-center gap-10 relative z-10 text-center md:text-right">
          <div className="space-y-4">
            ...
            <p className="text-slate-400 font-bold text-xl max-w-xl leading-relaxed">
              {isRTL 
                ? 'هذه المبالغ من الأرباح التي يمكن اكتسابها لو تم تحقيق نظرية ولاء العملاء بشكل سنوي، حيث يتحول العميل المقيّم إلى مسوق دائم لنشاطك.' 
                : 'Profits attainable through annual customer loyalty theory, where a reviewer becomes a lifelong promoter.'}
            </p>
          </div>
      ...
      {/* 7. الأزرار النهائية (الطلب والتجربة البصرية) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6">
        {/* زر الطلب - يوجه للواتساب مباشرة */}
        <button 
